 ...
 */
export async function waitForRuntimePanel(page: Page, timeout: number = 5000): Promise<void> {
	await expect(page.locator('[data-testid="runtime-panel"]')).toBeVisible({ timeout });
...
export async function waitForTabContent(
	...
	await expect(page.locator(`[data-testid="${tabName}-panel"]`)).toBeVisible({ timeout });
}
 ...
 */
export async function deleteTestFlow(page: Page, flowId: string): Promise<void> {
