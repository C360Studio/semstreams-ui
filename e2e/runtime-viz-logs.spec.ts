...
test.describe('Runtime Panel - Logs Tab', () => {
	...
	test('should connect to SSE endpoint when tab active', async ({ page }) => {
		...
		const connectingMessage = page.locator('[data-testid="connection-connecting"]');
	...
	test('should filter logs by level (DEBUG, INFO, WARN, ERROR)', async ({ page }) => {
	...
	test('should filter logs by component', async ({ page }) => {
		...
		if (logCount > 0) {
			...
			const componentOptions = options.filter((opt) => opt !== 'All Components');
			if (componentOptions.length > 0) {
				...
				const components = await logEntries.locator('.log-component').allTextContents();
	...
	test.skip('should show connection status (connecting, connected, disconnected)', async ({
		...
		const connectingStatus = page.locator('[data-testid="connection-connecting"]');
