...
import { getComponentTypes, type ComponentType } from './helpers/backend-helpers';
...
test.describe('Component Backend Integration', () => {
		...
		try {
			...
			if (schema.properties) {
				...
				for (const [fieldName, fieldSchema] of Object.entries(schema.properties)) {
					...
					await expect(label).toContainText(expectedLabel, { ignoreCase: true });
				}
			}
		} finally {
			// Cleanup
	...
	test('Port types from backend render correctly', async ({ page }) => {
		...
		const backendTypes = await getComponentTypes(page);
		const roboticsProcessor = backendTypes.find((t) => t.id === 'robotics-processor');
	...
	test('Component type protocol information is preserved', async ({ page }) => {
		...
		const websocketComponent = backendTypes.find((t) => t.protocol === 'websocket');
		...
		try {
			...
			await expect(websocketNode).toHaveAttribute('data-node-type', websocketComponent!.id);
		} finally {
			// Cleanup
			await deleteTestFlow(page, flowId);
		}
	...
	test('Backend component type schema validation errors surface in UI', async ({ page }) => {
