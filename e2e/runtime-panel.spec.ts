...
test.describe('Runtime Panel - WebSocket Integration', () => {
	...
	test('should show logs from running components in LogsTab', async ({ page }) => {
		// Open runtime panel
		await page.click('[data-testid="debug-toggle-button"]');
		await waitForRuntimePanel(page);
	...
	test('should show health updates for running components', async ({ page }) => {
		// Open runtime panel
		await page.click('[data-testid="debug-toggle-button"]');
		await waitForRuntimePanel(page);
	...
	test('should stop updates when flow is stopped', async ({ page }) => {
		...
		const stopResponse = await page.request.post(`/flowbuilder/deployment/${flowId}/stop`);
		if (stopResponse.ok()) {
	...
	test('should maintain data in correct tabs after tab switching', async ({ page }) => {
		// Open runtime panel
		await page.click('[data-testid="debug-toggle-button"]');
		await waitForRuntimePanel(page);
