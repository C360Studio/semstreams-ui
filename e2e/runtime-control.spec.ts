...
test.describe('Runtime Control', () => {
	test('Scenario 15: Deploy flow → Status becomes "deployed_stopped"', async ({ page }) => {
	...
	test('Scenario 16: Start flow → Status becomes "running"', async ({ page }) => {
		// Navigate to flow list
		const flowList = new FlowListPage(page);
		await flowList.goto();
		await page.waitForLoadState('networkidle');
	...
	test('Scenario 17: Stop flow → Status becomes "deployed_stopped"', async ({ page }) => {
		// Navigate to flow list
		const flowList = new FlowListPage(page);
		await flowList.goto();
		await page.waitForLoadState('networkidle');
	...
	test('Scenario 18: Edit running flow → Blocked with message', async ({ page }) => {
		// Navigate to flow list
		const flowList = new FlowListPage(page);
		await flowList.goto();
		await page.waitForLoadState('networkidle');
