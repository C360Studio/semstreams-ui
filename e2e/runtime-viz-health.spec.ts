...
test.describe('Runtime Panel - Health Tab', () => {
	...
	test('should display component health list with status indicators', async ({ page }) => {
		...
		if (rowCount > 0) {
			...
			const statusIndicator = firstRow.locator('[data-testid="status-indicator"]');
			...
			expect(['running', 'degraded', 'error']).toContain(statusText?.toLowerCase() || '');
	...
	test('should show last activity timestamps', async ({ page }) => {
		...
		if (cellCount > 0) {
			...
			const isStale = await firstCell.evaluate((el) => el.classList.contains('stale'));
			if (isStale) {
				// Stale components should have warning color
				const color = await firstCell.evaluate((el) => window.getComputedStyle(el).color);
				expect(color).toBeTruthy();
			}
		}
	...
	test('should expand component details on click (when degraded/error)', async ({ page }) => {
		// Wait for health data
		await page.waitForTimeout(1000);
		...
		if (buttonCount > 0) {
			...
			const expandedState = await firstExpandButton.getAttribute('aria-expanded');
			...
			const collapsedState = await firstExpandButton.getAttribute('aria-expanded');
			expect(collapsedState).toBe('false');
		}
		// else: No components with details - test passes (all components healthy)
	});

	test('should show health details (error messages, diagnostics)', async ({ page }) => {
		// Wait for health data
		await page.waitForTimeout(1000);
	...
	test('should handle backend health endpoint unavailable', async ({ page }) => {
