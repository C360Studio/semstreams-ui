...
test.describe('Deploy Blocking', () => {
	...
	test('should disable deploy button when flow has validation errors', async ({ page }) => {
	...
	test('should show error modal when attempting to deploy with validation errors', async ({ page }) => {
		const flowList = new FlowListPage(page);
		await flowList.goto();
		await page.waitForLoadState('networkidle');
					...
					nodes: [
						{
							...
							ports: [
								{ name: 'nats_input', direction: 'input', required: true, validation_state: 'error' },
								{ name: 'entities_output', direction: 'output', required: true, validation_state: 'error' }
							]
						}
					],
					discovered_connections: []
					...
					nodes: [
						{
							id: 'udp-1',
							ports: [
								{ name: 'udp_socket', direction: 'input', required: true, validation_state: 'valid' },
								{ name: 'nats_output', direction: 'output', required: true, validation_state: 'valid' }
							]
						},
						{
							id: 'robotics-1',
							ports: [
								{ name: 'nats_input', direction: 'input', required: true, validation_state: 'valid' },
								{ name: 'entities_output', direction: 'output', required: true, validation_state: 'valid' }
							]
						}
					],
					discovered_connections: [
