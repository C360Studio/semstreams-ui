...
test.describe('Port Configuration (Schema Tag System)', () => {
	...
	test('Scenario 4: Edit port subject → Value updates', async ({ page }) => {
		...
		await configPanel.fillPortField('output', 0, 'subject', 'events.udp.custom');

		// Wait for input to update
		await page.waitForTimeout(100);

		// Verify the value persisted
		await configPanel.expectPortFieldValue('output', 0, 'subject', 'events.udp.custom');
	});
	...
	test('Scenario 6: Configure ports → Save → Values persist', async ({ page }) => {
		...
		await configPanel.expectPortCount('output', 1);
		await configPanel.expectPortFieldValue('output', 0, 'subject', 'events.test.output');
	});
	...
	test('Scenario 7: Multiple ports → All editable independently', async ({ page }) => {
		...
		await configPanel.expectPortFieldValue('output', 0, 'subject', 'events.port1');
		await configPanel.expectPortFieldValue('output', 1, 'subject', 'events.port2');
		await configPanel.expectPortFieldValue('output', 2, 'subject', 'events.port3');
	});
	...
	test('Scenario 8: Full user journey with port configuration', async ({ page }) => {
		...
		await configPanel.clickAddInputPort();
		await configPanel.fillPortField('input', 0, 'subject', 'udp://0.0.0.0:14550');
		...
		await configPanel.expectPortCount('input', 1);
		await configPanel.expectPortFieldValue('input', 0, 'subject', 'udp://0.0.0.0:14550');

		// 11. Verify output port persisted
		await configPanel.expectPortCount('output', 1);
		await configPanel.expectPortFieldValue('output', 0, 'subject', 'telemetry.mavlink');
	});
