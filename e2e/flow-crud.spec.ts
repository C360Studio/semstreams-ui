...
test.describe('Flow CRUD Operations', () => {
	...
	test('Edit flow (add components) → verify saved and persisted', async ({ page }) => {
		// Create flow via API
	...
	test('Edit flow (add multiple components) → verify all saved', async ({ page }) => {
		// Create flow via API
		const { id: flowId, url } = await createTestFlow(page, {
	...
	test('Edit flow (add connections) → verify saved and persisted', async ({ page }) => {
	...
	test('Delete flow via API → verify removed from backend', async ({ page }) => {
		// Create flow via API
	...
	test('List flows shows all backend flows', async ({ page }) => {
		...
		const backendTestFlows = backendFlows.filter((f) => testFlowIds.includes(f.id));
	...
	test('Flow list pagination and sorting (if implemented)', async ({ page }) => {
		...
		const backendFlows = await listFlows(page);
		const testFlows = backendFlows.filter((f: FlowSummary) => f.name.startsWith('E2E Test Flow'));
