...
export class StatusBarPage {
	...
	async expectRuntimeState(state: 'not_deployed' | 'deployed_stopped' | 'running' | 'error'): Promise<void> {
	...
	async expectStateTransition(fromState: string, toState: string, timeoutMs: number = 5000): Promise<void> {
		// Wait for state to change from initial state
		await expect(this.runtimeState).not.toHaveAttribute('data-state', fromState, { timeout: timeoutMs });
