...
test.describe('Runtime Panel - Metrics Tab', () => {
	...
	test('should poll metrics endpoint at regular intervals', async ({ page }) => {
	...
	test('should display component metrics table (throughput, error rate)', async ({ page }) => {
	...
	test('should show status indicators (healthy, degraded, error)', async ({ page }) => {
		// Wait for metrics
		await page.waitForTimeout(1000);
	...
	test('should show "No data" when metrics unavailable', async ({ page }) => {
		...
		if (!hasTable) {
			...
			if (hasEmpty) {
				...
				throw new Error('Expected empty state or error message when metrics unavailable');
			}
		}
