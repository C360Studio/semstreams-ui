...
import { createTestFlow, deleteTestFlow, createFlowWithValidationErrors } from './helpers/flow-setup';
...
test.describe('Deployment Lifecycle - Backend Verification', () => {
	...
	test('Deploy flow verifies backend runtime_state is "deployed_stopped"', async ({ page }) => {
	...
	test('Start flow verifies backend runtime_state is "running"', async ({ page }) => {
	...
	test('Stop flow verifies backend runtime_state is "deployed_stopped"', async ({ page }) => {
	...
	test('Undeploy flow verifies backend runtime_state is "not_deployed"', async ({ page }) => {
		// Create and deploy a flow
		const { id, url } = await createTestFlow(page, {
	...
	test('Invalid flow blocks deployment and backend state stays "not_deployed"', async ({ page }) => {
		...
		const validationDialog = page.locator('[role="dialog"]').or(page.locator('.dialog')).or(page.locator('.modal'));

		// Check if dialog appears
		const dialogVisible = await validationDialog.isVisible({ timeout: 2000 }).catch(() => false);

		if (dialogVisible) {
			// Verify validation message is shown
			await expect(validationDialog).toContainText(/validation|error|invalid|configuration/i);
		} else {
			// Alternatively, check for error message in status bar or toast
			const errorMessage = page.locator('.error-message, .status-message, .toast');
			await expect(errorMessage.first()).toContainText(/validation|error|invalid|configuration/i, { timeout: 2000 });
		}
	...
	test('Cannot start undeployed flow', async ({ page }) => {
		...
		const startButtonVisible = await startButton.isVisible({ timeout: 1000 }).catch(() => false);
