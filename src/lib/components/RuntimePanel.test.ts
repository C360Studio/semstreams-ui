...
describe('RuntimePanel', () => {
	...
	describe('Tab Navigation', () => {
		...
		it('should switch to Metrics tab when clicked', async () => {
			...
			global.fetch = vi.fn().mockResolvedValue({
				ok: true,
				json: async () => ({ timestamp: '2025-11-17T14:23:05.123Z', components: [] })
			});
	...
	describe('Panel Height', () => {
		...
		it('should use custom height when provided', () => {
			render(RuntimePanel, { props: { isOpen: true, height: 400, flowId: 'test-flow' } });
	...
	describe('Close Functionality', () => {
		it('should have close button', () => {
			...
			expect(screen.getByRole('button', { name: /close runtime panel/i })).toBeInTheDocument();
		});

		it('should call onClose when close button clicked', async () => {
			const onClose = vi.fn();
			render(RuntimePanel, { props: { isOpen: true, onClose, flowId: 'test-flow' } });

			const closeButton = screen.getByRole('button', { name: /close runtime panel/i });
		...
		it('should not call onClose when Esc pressed and panel is closed', async () => {
			const onClose = vi.fn();
			render(RuntimePanel, { props: { isOpen: false, onClose, flowId: 'test-flow' } });
