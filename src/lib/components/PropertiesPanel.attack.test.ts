...
describe('PropertiesPanel Attack Tests', () => {
	describe('Accessibility Attacks', () => {
		it('handles keyboard-only navigation for search input', async () => {
			const componentType: ComponentType = {
				...
				schema: {
					type: 'object',
					properties: {
						field1: { type: 'string', description: 'Field 1', category: 'basic' },
						field2: { type: 'string', description: 'Field 2', category: 'advanced' }
					},
					required: []
				}
			};
		...
		it('handles keyboard navigation in delete confirmation modal', async () => {
			...
			const cancelButton = modal?.querySelector('.btn-secondary') as HTMLButtonElement;
			const confirmButton = modal?.querySelector('.btn-danger') as HTMLButtonElement;
		...
		it('handles special characters in field names', async () => {
			const componentType: ComponentType = {
				...
				schema: {
					type: 'object',
					properties: {
						'field-with-dashes': { type: 'string', description: 'Dashes', category: 'basic' },
						'field.with.dots': { type: 'string', description: 'Dots', category: 'basic' },
						'field$with$dollars': { type: 'string', description: 'Dollars', category: 'basic' }
					},
					required: []
				}
			};
		...
		it('handles rapid open/close of advanced section', async () => {
			const componentType: ComponentType = {
				...
				schema: {
					type: 'object',
					properties: {
						basic_field: { type: 'string', description: 'Basic', category: 'basic' },
						advanced_field: { type: 'string', description: 'Advanced', category: 'advanced' }
					},
					required: []
				}
			};
	...
	describe('Effect Cleanup Attacks', () => {
		...
		it('handles delete modal cancellation properly', async () => {
			...
			expect(container.querySelector('[data-testid="delete-confirm"]')).toBeInTheDocument();
			...
			expect(container.querySelector('[data-testid="delete-confirm"]')).not.toBeInTheDocument();
		});
		...
		it('handles clearing search query manually', async () => {
			const componentType: ComponentType = {
				...
				schema: {
					...
					properties: {
						field1: { type: 'string', description: 'Field 1', category: 'basic' },
						field2: { type: 'string', description: 'Field 2', category: 'basic' }
					},
					required: []
				}
			};
	...
	describe('XSS Prevention Attacks', () => {
		it('escapes malicious script in field names', () => {
			...
			const label = container.querySelector('label[for="<script>alert(\\"XSS\\")</script>"]');
