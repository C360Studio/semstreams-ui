...
function createHealthComponent(overrides: Partial<ComponentHealth> = {}): ComponentHealth {
	return {
		name: 'test-component',
		component: 'test',
		type: 'processor',
...
function createHealthOverall(overrides: Partial<HealthOverall> = {}): HealthOverall {
	return {
		status: 'healthy',
		counts: {
			healthy: 3,
	...
	describe('Component Health Table', () => {
		...
		it('should show healthy status with green indicator', async () => {
			...
			await waitFor(() => {
				...
				const healthyIndicators = indicators.filter((el) => el.textContent === '●');
