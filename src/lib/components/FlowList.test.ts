...
describe('FlowList', () => {
	const createMockFlow = (id: string, name: string, runtimeState: Flow['runtime_state'] = 'not_deployed'): Flow => ({
		version: 1,
		id,
		name,
	...
	it('should call onCreate when "Create New Flow" button is clicked', async () => {
		...
		const createButton = screen.getByRole('button', { name: /create new flow/i });
	...
	it('should show Create button even when flows exist', () => {
		...
		expect(screen.getByRole('button', { name: /create new flow/i })).toBeInTheDocument();
	});
