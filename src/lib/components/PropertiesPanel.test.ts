...
describe('PropertiesPanel - Collapsible Sections', () => {
	...
	it('should render basic fields in always-visible section', () => {
		...
		const basicSection = container.querySelector('[data-testid="basic-fields"]');
	...
	it('should render advanced fields in collapsible details element', () => {
		...
		const advancedSection = container.querySelector('details[data-testid="advanced-fields"]');
	...
	it('should sort fields alphabetically within each category', () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					zebra_field: { type: 'string', description: 'Z field', category: 'basic' },
					alpha_field: { type: 'string', description: 'A field', category: 'basic' },
					beta_field: { type: 'string', description: 'B field', category: 'basic' }
				},
				required: []
			}
		};
	...
	it('should not render basic section if no basic fields', () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					buffer_size: { type: 'int', description: 'Buffer', category: 'advanced' }
				},
				required: []
			}
		};
...
describe('PropertiesPanel - Search/Filter', () => {
	const createComponentTypeWithManyFields = (): ComponentType => ({
		...
		schema: {
			...
			properties: {
				...
				buffer_size: { type: 'int', description: 'Buffer size in bytes', category: 'advanced' },
				timeout: { type: 'int', description: 'Connection timeout', category: 'advanced' },
				retry_count: { type: 'int', description: 'Number of retries', category: 'advanced' }
			},
			required: []
		}
	});
	...
	it('should show no results message when no fields match', async () => {
		...
		const noResults = container.querySelector('[data-testid="no-search-results"]');
...
describe('PropertiesPanel - Field Name Humanization', () => {
	it('should display humanized field names as labels', () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					bind_address: { type: 'string', description: 'Address', category: 'basic' },
					udp_port: { type: 'int', description: 'Port', category: 'basic' },
					max_buffer_size: { type: 'int', description: 'Buffer', category: 'basic' }
				},
				required: []
			}
		};
		...
		const bindAddressLabel = container.querySelector('label[for="bind_address"]');
	...
	it('should preserve acronyms in uppercase', () => {
		const componentType: ComponentType = {
			...
			schema: {
				...
				properties: {
					...
					http_timeout: { type: 'int', description: 'Timeout', category: 'basic' },
		...
		expect(container.querySelector('label[for="ip_address"]')).toHaveTextContent('IP Address');
		expect(container.querySelector('label[for="http_timeout"]')).toHaveTextContent(
			'HTTP Timeout'
		);
		expect(container.querySelector('label[for="api_key"]')).toHaveTextContent('API Key');
		expect(container.querySelector('label[for="ttl_seconds"]')).toHaveTextContent(
			'TTL Seconds'
		);
	});
});
...
describe('PropertiesPanel - SchemaField Integration', () => {
	it('should use SchemaField component for string fields', () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					name: { type: 'string', description: 'Name field', category: 'basic' }
				},
				required: []
			}
		};
	...
	it('should use SchemaField component for int fields', () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					port: { type: 'int', description: 'Port', category: 'basic', minimum: 1, maximum: 65535 }
				},
				required: []
			}
		};
	...
	it('should use SchemaField component for ports fields', () => {
		...
		const portsSection = container.querySelector('[data-testid="port-config-editor"]');
	...
	it('should pass validation errors to SchemaField', async () => {
		const componentType: ComponentType = {
			...
			schema: {
				type: 'object',
				properties: {
					port: { type: 'int', description: 'Port', category: 'basic', minimum: 1, maximum: 100 }
				},
				required: ['port']
			}
		};
...
describe('PropertiesPanel - Existing Functionality Preserved', () => {
	it('should still render empty state when mode is empty', () => {
		...
		const emptyState = container.querySelector('[data-testid="properties-empty"]');
	...
	it('should still render type preview when mode is type-preview', () => {
		...
		const preview = container.querySelector('[data-testid="properties-type-preview"]');
	...
	it('should still show delete confirmation dialog', async () => {
		...
		const confirmDialog = container.querySelector('[data-testid="delete-confirm"]');
