...
describe('ValidationErrorDialog', () => {
		...
		errors: [
			{
				...
				suggestions: ['Connect to a processor component', 'Add WebSocket Output to view data']
			},
			{
				type: 'orphaned_port',
				severity: 'error',
	...
	describe('DOM Output', () => {
		it('should render error messages grouped by component', () => {
			...
			expect(container).toHaveTextContent('Required output port has no subscribers');
			expect(container).toHaveTextContent('Required input port has no publishers');
		...
		it('should render suggestions when present', () => {
			...
			expect(container).toHaveTextContent('Connect an output from another component');
		});
		...
		it('should render warnings section when present', () => {
			...
			expect(container).toHaveTextContent('Optional output port has no subscribers');
		});
