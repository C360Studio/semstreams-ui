...
describe('ConfigPanel (Prop-Based Architecture)', () => {
	...
	describe('Config Display', () => {
		...
		it('should display config as JSON in textarea when schema unavailable', async () => {
			...
			await waitFor(() => {
				const textarea = screen.queryByRole('textbox', { name: /configuration/i });
				expect(textarea).toBeInTheDocument();
			});

			const textarea = screen.getByRole('textbox', { name: /configuration/i });
			expect(textarea).toHaveValue(JSON.stringify(mockComponent.config, null, 2));
		});
	});
	...
	describe('Config Editing', () => {
		it('should call onSave when Save button is clicked', async () => {
			...
			await waitFor(() => {
				expect(screen.queryByRole('textbox', { name: /configuration/i })).toBeInTheDocument();
			});

			const textarea = screen.getByRole('textbox', { name: /configuration/i });
			const newConfig = { port: 14551, bind_address: '127.0.0.1' };
			await fireEvent.input(textarea, { target: { value: JSON.stringify(newConfig, null, 2) } });
		...
		it('should call onClose when Save is successful', async () => {
			...
			render(ConfigPanel, { props: { component: mockComponent, onSave, onClose } });
		...
		it('should handle JSON editor for complex config', async () => {
			...
			const jsonEditor = screen.getByRole('textbox', { name: /configuration/i });
			...
			await fireEvent.input(jsonEditor, { target: { value: JSON.stringify(newConfig) } });
	...
	describe('Validation', () => {
		...
		it('should clear error when valid JSON is entered', async () => {
			...
			await fireEvent.input(jsonEditor, { target: { value: '{"port": 14551}' } });
	...
	describe('Edge Cases', () => {
		...
		it('should handle component switching', async () => {
			const { rerender } = render(ConfigPanel, { props: { component: mockComponent } });
		...
		it('should clear panel when component is deselected', async () => {
			const { rerender, container } = render(ConfigPanel, { props: { component: mockComponent } });
		...
		it('should reset config when Cancel is clicked after editing', async () => {
			...
			const textarea = screen.getByRole('textbox', { name: /configuration/i }) as HTMLTextAreaElement;
	...
	describe('Schema Extraction from API Response (T004)', () => {
		it('should handle full backend API response format', async () => {
			...
			const mockApiResponse = {
				...
				schema: {
					properties: {
						port: { type: 'int', default: 14550, category: 'basic', description: 'Port number' }
					},
					required: ['port']
				}
			};
	...
	describe('Dirty State Preservation (T043)', () => {
		it('should preserve dirty state when switching components', async () => {
			...
			const textarea = screen.getByRole('textbox', { name: /configuration/i }) as HTMLTextAreaElement;
			const dirtyConfig = { port: 8080, bind_address: '127.0.0.1', custom_field: 'test' };
			await fireEvent.input(textarea, { target: { value: JSON.stringify(dirtyConfig, null, 2) } });
			...
			const textareaAfter = screen.getByRole('textbox', { name: /configuration/i }) as HTMLTextAreaElement;
		...
		it('should clear dirty state after successful save', async () => {
			...
			await fireEvent.input(textarea, { target: { value: JSON.stringify(newConfig) } });
