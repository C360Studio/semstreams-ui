...
describe('SchemaForm', () => {
	...
	it('should pre-fill fields with schema defaults', async () => {
		...
		const bindInput = container.querySelector('input#bind_address') as HTMLInputElement;
		expect(bindInput?.value).toBe('0.0.0.0');

		const enabledInput = container.querySelector('input#enabled') as HTMLInputElement;
