...
describe('ComponentPalette', () => {
	...
	it('should display component descriptions', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('Receives MAVLink messages via UDP')).toBeInTheDocument();
			expect(screen.getByText('Streams data via WebSocket')).toBeInTheDocument();
		});
	});
	...
	it('should group components by category', async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByText('input', { selector: '.category-header' })).toBeInTheDocument();
			expect(screen.getByText('output', { selector: '.category-header' })).toBeInTheDocument();
		});
	});
	...
	it('should make component cards draggable', async () => {
		...
		await waitFor(() => {
			const componentCard = screen.getByText('UDP Input').closest('[draggable="true"]');
	...
	it('should handle empty component list', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText(/component palette coming soon/i)).toBeInTheDocument();
		});
	});
	...
	it('should display component categories correctly', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('input', { selector: '.category-header' })).toBeInTheDocument();
			expect(screen.getByText('output', { selector: '.category-header' })).toBeInTheDocument();
			expect(screen.getByText('processor', { selector: '.category-header' })).toBeInTheDocument();
		});
	});
});
