...
describe('deploymentApi', () => {
	...
	describe('deployFlow', () => {
		it('should deploy a flow', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('/flowbuilder/deployment/flow-123/deploy', {
				method: 'POST'
			});
		});
		...
		it('should handle deploy errors', async () => {
			...
			try {
				...
				expect((error as DeploymentApiError).details).toEqual({ error: 'Invalid flow state' });
			}
		});
	...
	describe('startFlow', () => {
		it('should start a deployed flow', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('/flowbuilder/deployment/flow-123/start', {
				method: 'POST'
			});
		});
		...
		it('should handle start errors (flow not deployed)', async () => {
			...
			try {
				...
				expect((error as DeploymentApiError).message).toContain('Failed to start flow');
			}
		});
	...
	describe('stopFlow', () => {
		it('should stop a running flow', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('/flowbuilder/deployment/flow-123/stop', {
				method: 'POST'
			});
		});
	...
	describe('undeployFlow', () => {
		it('should undeploy a stopped flow', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('/flowbuilder/deployment/flow-123/undeploy', {
				method: 'POST'
			});
		});
	...
	describe('DeploymentApiError', () => {
		...
		it('should create error with details', () => {
			...
			const error = new DeploymentApiError('Operation failed', 'start', 400, details);
