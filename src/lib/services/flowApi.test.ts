...
describe('flowApi', () => {
	...
	describe('createFlow', () => {
		...
		it('should handle creation errors', async () => {
			...
			try {
				...
				expect((error as FlowApiError).details).toEqual({ error: 'Invalid name' });
			}
		});
	...
	describe('listFlows', () => {
		it('should list all flows', async () => {
			const mockFlows = [createMockFlow(), { ...createMockFlow(), id: 'flow-456' }];
			mockFetch.mockResolvedValueOnce({
				ok: true,
		...
		it('should handle list errors', async () => {
			...
			try {
				...
				expect((error as FlowApiError).message).toContain('Failed to list flows');
			}
		});
	...
	describe('updateFlow', () => {
		...
		it('should handle version conflicts (409)', async () => {
			...
			try {
				...
				expect((error as FlowApiError).message).toContain('Failed to update flow');
			}
		});
		...
		it('should handle update errors', async () => {
			...
			await expect(flowApi.updateFlow('flow-123', mockFlow)).rejects.toThrow(FlowApiError);
		});
	...
	describe('deleteFlow', () => {
		...
		it('should handle delete errors', async () => {
			...
			try {
				...
				expect(error).toBeInstanceOf(FlowApiError);
				expect((error as FlowApiError).statusCode).toBe(404);
				expect((error as FlowApiError).message).toContain('Failed to delete flow');
			}
		});
