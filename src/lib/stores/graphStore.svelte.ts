...
function createGraphStore() {
	...
	const { subscribe, update, set } = store.writable<GraphStoreState>(initialState);
	...
	return {
		...
		getFilteredEntities(state: GraphStoreState): GraphEntity[] {
			...
			if (filters.domains.length > 0) {
				result = result.filter((e) => filters.domains.includes(e.idParts.domain));
			}
			...
			if (filters.timeRange) {
				...
				result = result.filter((e) => {
					...
					return e.properties.some((p) => p.timestamp >= start && p.timestamp <= end);
				});
			}
		...
		getFilteredRelationships(state: GraphStoreState): GraphRelationship[] {
			...
			const visibleEntityIds = new Set(this.getFilteredEntities(state).map((e) => e.id));
