	...
	describe('RuntimeStateInfo', () => {
		it('should have correct structure with all runtime states', () => {
			const states: RuntimeStateInfo[] = [
				...
				{ state: 'deployed_stopped', message: null, lastTransition: new Date() },
				{ state: 'running', message: 'All components running', lastTransition: new Date() },
				{ state: 'error', message: 'Component crashed', lastTransition: new Date() }
			];

			states.forEach((state) => {
				expect(state.state).toMatch(/not_deployed|deployed_stopped|running|error/);
