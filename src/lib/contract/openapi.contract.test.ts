 ...
 */
function loadOpenAPISpec(): any | null {
	try {
...
To enable these tests:
  ...
  2. In a monorepo setup:
     ...
     npm test
		...
		if (err.message.includes('OpenAPI spec not found')) {
 ...
 */
function loadSchemaFile(filename: string): any {
...
For more info, see your backend's documentation on schema generation
		...
		throw new Error(`Failed to parse schema ${filename}: ${err.message}`);
	}
}

describe.skipIf(!existsSync(OPENAPI_SPEC_PATH))('OpenAPI Specification Contract Tests', () => {
	let openapiSpec: any;
	...
	it('should have ComponentType schema with component references', () => {
		...
		expect(Array.isArray(componentType.properties.schema.oneOf)).toBe(true);
		expect(componentType.properties.schema.oneOf.length).toBeGreaterThan(0);
	});
...
describe.skipIf(!existsSync(OPENAPI_SPEC_PATH))('OpenAPI Schema References', () => {
